...
# endif
   ...
   SUBROUTINE nemo_init
      ...
      NAMELIST/nammpp/ jpni, jpnj, nn_hls, ln_nnogather, ln_listonly, nn_comm, &
           &             ln_ensemble, ln_ens_rst_in, nn_ens_size, nn_ens_start, ln_ens_diag,ln_sto_atmo
      ! ENSEMBLE ]
      !!----------------------------------------------------------------------
      !
      cxios_context = 'nemo'
      ...
      ! open reference and configuration namelist files
      CALL load_nml_ens( numnam_ref,        'namelist_ref',                                           -1, .true. )
      CALL load_nml_ens( numnam_cfg,        'namelist_cfg',                                           -1, .true. )
      ...
      905   IF( ios /= 0 ) CALL ctl_nam ( ios , 'nammpp in reference namelist' )
      ...
      READ  ( numnam_cfg, nammpp, IOSTAT = ios, ERR = 906 )
       906   IF( ios /= 0 ) CALL ctl_nam ( ios , 'nammpp in configuration namelist' )
      ! ENSEMBLE ]
      !                             !-------------------------------------------------!
      ...
      IF( Agrif_Root() ) THEN
         ...
         ENDIF
      ENDIF
      ! ENSEMBLE: replace line: CALL mpp_start( ilocal_comm ) by IF block below:
      ! ENSEMBLE [
      IF( ln_ensemble ) THEN
      ...
      IF( lk_oasis ) THEN
         ...
         CALL mpp_start( ilocal_comm )
      ELSE
      ! ENSEMBLE : replace line:   CALL mpp_start( ) by IF block below:
      ! ENSEMBLE [
         IF( ln_ensemble ) THEN
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namctl in configuration namelist' )
      ...
      IF(lwp) THEN                      ! open listing units
         !
         ! ENSEMBLE [    replaced ocean.output by cloutput in the lines below

...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namrun in configuration namelist' )
      ...
      ! ENSEMBLE [
      ! SLX: i commented off those lines as it is not needed in my case (?)
      IF(ln_ens_rst_in) cn_ocerst_in = cn_member//cn_ocerst_in
      !!! IF(ln_ensemble) cn_ocerst_out = cn_member//cn_ocerst_out
      ! ENSEMBLE ]
      ...
      ! ENSEMBLE [
      IF(ln_ensemble) cexper = cn_member//cexper
      ! ENSEMBLE ]
      nrstdt = nn_rstctl
      nit000 = nn_it000
      nitend = nn_itend

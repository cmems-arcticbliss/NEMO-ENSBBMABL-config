MODULE sbcabl
   ...
   USE dom_oce, ONLY  : tmask,cn_member 
   !
...
#if defined key_si3
   USE ice    , ONLY : u_ice, v_ice, tm_su, ato_i,ln_rhg_BBM,utauVice,vtauUice !!!  SLX added ln_rhg_BBM   ! ato_i = total open water fractional area
         ...
         IF( ln_rhg_BBM ) THEN
           CALL abl_stp( kt, tsk_m, ssu_m, ssv_m, zssq,                          &   !   <<= in
            ...
            &            , utau_ice, vtau_ice                                  &   !   =>> out
                         , utauVice=utauVice, vtauUice=vtauUice            &   ! optional out (BBM rheology)
#endif
            &                                                                  )
         ELSE
           CALL abl_stp( kt, tsk_m, ssu_m, ssv_m, zssq,                          &   !   <<= in

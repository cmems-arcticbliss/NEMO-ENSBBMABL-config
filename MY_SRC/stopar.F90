...
902   IF( ios >  0 ) CALL ctl_nam ( ios , 'namsto in configuration namelist' )
      ...
      ! SLX
      print*,"=== in stopar ",ln_sto_atmo
      IF( .NOT.ln_sto_atmo ) THEN   ! no use of perturbed atmospheric forcing
         IF(lwp) THEN
            WRITE(numout,*)
            WRITE(numout,*) 'sto_par_init : NO use of perturbed atmospheric forcing'
            WRITE(numout,*) '~~~~~~~~~~~~'
         ENDIF
      ELSE
         IF(lwp) THEN
            WRITE(numout,*)
            WRITE(numout,*) 'sto_par_init : Perturbed atmospheric forcing'
            WRITE(numout,*) '~~~~~~~~~~~~'
         ENDIF
      ENDIF
      ...
      ! ENSEMBLE [ 
      IF(ln_ens_rst_in) cn_storst_in = cn_member//cn_storst_in
      ! ENSEMBLE ]
      ! Parameter print
      IF(lwp) THEN
         WRITE(numout,*)
   ...
   SUBROUTINE sto_rst_write( kt )
      ...
      IF ( jpsto2d > 0 .OR. jpsto3d > 0 ) THEN
         ...
         IF( kt == nitrst - 1 .OR. nn_stock == 1 .OR. kt == nitend-1 ) THEN
            ...
            ! open the restart file
            ! [SLX
            !CALL iom_open( TRIM(clname), numstow, ldwrt = .TRUE. )
            CALL iom_open( TRIM(clpath)//TRIM(clname), numstow, ldwrt = .TRUE. )
            ! SLX ]
            ENDIF
         ENDIF

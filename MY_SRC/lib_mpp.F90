MODULE lib_mpp
   ...
   PRIVATE
   !
   PUBLIC   ctl_stop, ctl_warn, ctl_opn, ctl_nam, load_nml, load_nml_ens
   ! ENSEMBLE [
   PUBLIC   mpp_start,mpp_start_ensemble, mppstop, mppsync, mpp_comm_free
   ! ENSEMBLE ]
...
!$AGRIF_END_DO_NOT_TREAT
   ...
   LOGICAL, PUBLIC                                  :: ln_sto_atmo = .FALSE.   ! SLX: switch if perturbed atmo (to fix input fil names)
...
#  undef OPERATION_MAXLOC
   ...
   SUBROUTINE ctl_opn( knum, cdfile, cdstat, cdform, cdacce, klengh, kout, ldwp, karea )
      ...
      ELSE
         !LOLO:OPEN( UNIT=knum, FILE=clfile, FORM=cdform, ACCESS=cdacce, STATUS=cdstat                      , ERR=100, IOSTAT=iost )
         OPEN( UNIT=knum, FILE=clfile, FORM=cdform, ACCESS=cdacce, STATUS=cdstat, RECL=512            , ERR=100, IOSTAT=iost )  ; !lolo
      ENDIF
...
100   CONTINUE
   ...
   END FUNCTION get_unit
      ...
      ! ENSEMBLE ]
SUBROUTINE load_nml_ens( cdnambuff , cdnamfile, kout, ldwp)
    CHARACTER(LEN=:)    , ALLOCATABLE, INTENT(INOUT) :: cdnambuff
